...
const editProduct = (id: number, name: string, tag: string, version: number, price: number, sxcname: string, bbb_id: number, link: string, licenseda: boolean, isnewa: boolean, autoinstallera: boolean, recurrenta: boolean, taba: boolean, tabroute: string, description: string, hidea: boolean, extension: boolean, slug: string, category: string, isWings: boolean, extension_product?: number | null, logo?: File | null, zip?: File | null): Promise<any> => {
    ...
    return new Promise<any>((resolve, reject) => {
        httpMultipart
            .post(`${config.privateapilink}/admin/products/${id}`, { name, tag, version, price, sxcname, bbb_id, link, licensed: licensedString, isnew: isNewString, autoinstaller: autoinstallerString, recurrent: recurrentString, tab: tabString, tabroute, description, hide: hideString, slug: slug, category: category, isWings: isWingsString, logo, zip, extension: extensionString, extension_product})
